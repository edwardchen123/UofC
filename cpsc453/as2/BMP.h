...
class BMPImg
 {
  public:
   ...
   int Load(char const* szFilename);
   int GetBPP();
   int GetWidth();
   int GetHeight();
  ...
  private:
   ...
   // Internal workers
   int GetFile(char const* szFilename);
   int ReadBmpHeader();
   int LoadBmpRaw();
   int LoadBmpRLE8();
   int LoadBmpPalette();
...
inline int BMPImg::Load(char const* szFilename)
 {
  int iRet;
...
inline int BMPImg::GetFile(char const* szFilename)
 {
  using namespace std;
  ifstream fIn;
