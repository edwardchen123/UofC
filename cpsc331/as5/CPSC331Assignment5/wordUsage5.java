...
/**
 *
 * Displays the words found in a specified text file, listed with
 * most frequently appearing words first, and using dictionary order
 * to break ties, along with the number of occurrences of each;
    ...
    if (args.length == 1) {
      ...
      try {
       ...
       inputSource words = new inputSource(args[0]);
        ...
        ArrayList<Pair<String, Integer>> sortedWords
        ...
        pairOrdering<String, Integer> frequencyOrder
                     = new pairOrdering<String, Integer>();

        mySort.sort(sortedWords, frequencyOrder);

        int i = 0;

        for (Pair<String, Integer> current: sortedWords) {
