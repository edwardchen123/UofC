...
/**
 * Encapsulates useful information 
 **/
typedef struct {
...
int width,height;
int my_video_ds;
unsigned char doubleBuff[768*1024];
unsigned char prevBuff[768*1024];
...
void printHeader() {
        ...
        drawText(XOFF,XOFF,title,WHITE,2);
        drawText(XOFF,XOFF+20,authors,WHITE,1);
        drawText(700,XOFF,insLabel,WHITE,2);
        drawText(700,XOFF+20,enter,WHITE,1);
        drawText(700,XOFF+30,arrows,WHITE,1);
        drawText(700,XOFF+40,space,WHITE,1);
        drawText(700,XOFF+50,esc,WHITE,1);
}
...
void drawPixel(int x, int y, char color){
   // _farpokeb(my_video_ds, (y*width)+x, color);
	doubleBuff[(y*width)+x] = color;
}

void drawScaledPixel(int x, int y, int scale, char color) {
	int i;
	for (i = 0; i < scale; i++) {
		drawHLine(x, y+i, scale, color);
	}
}
